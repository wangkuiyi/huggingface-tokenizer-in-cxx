...
SRC_DIR=$SCRIPT_DIR/re2
BUILD_DIR=$SCRIPT_DIR/build/re2
XCFRAMEWORK="$BUILD_DIR"/re2.xcframework


if [[ $FORCE_REBUILD == "YES" ]]; then
    echo "┌------------------------------------------------------------------------------┐"
    echo "  Deleting existig RE2 frameworks ..."
    echo "└------------------------------------------------------------------------------┘"
    rm -rf "$BUILD_DIR"
fi
...
function build_for_ios() {
    ...
    *)
    ...
    test_file="$build_dir"/re2.framework/re2
...
function merge_fat_static_library() {
    ...
    src="$BUILD_DIR"/$src_label/re2/re2.framework/re2
    dst="$BUILD_DIR"/$dst_label/re2/re2.framework/re2
...
xcodebuild -create-xcframework \
    -framework "$BUILD_DIR"/macos-arm64/re2/re2.framework \
    -framework "$BUILD_DIR"/ios-sim-arm64/re2/re2.framework \
    -framework "$BUILD_DIR"/ios-dev-arm64/re2/re2.framework \
    -output "$XCFRAMEWORK"
